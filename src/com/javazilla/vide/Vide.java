...
import static com.javazilla.vide.Vide.Version.VERSION;
import static com.javazilla.vide.Vide.Version.V_COMMIT;
import static com.javazilla.vide.Vide.Version.V_FULL;
...
public class Vide {
    ...
    public static class Version {
        ...
        public static double    V_MAJOR_MINOR;
        public static int       V_PATCH;
        public static String    V_FULL;
        public static String    V_COMMIT;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        String vver = VCmdRunner.runV("version").substring(2).trim();
        Version.V_COMMIT = vver.split(" ")[1];
        String[] vver_ = vver.split(" ")[0].split("[.]");

        Version.V_MAJOR_MINOR = Double.parseDouble(vver_[0] + "." + vver_[1]);
        if (vver_.length > 2) Version.V_PATCH = Integer.parseInt(vver_[2]);
        V_FULL = Version.V_MAJOR_MINOR + "." + Version.V_PATCH + " (" + V_COMMIT + ")";

        System.out.println("V " + vver);

        JFrame f = new JFrame("VIDE " + VERSION + " (V " + V_FULL + ")");
        ...
        final MyFile mf = new MyFile(file.getAbsoluteFile().getParentFile().getParentFile());
        model = new FileTreeModel(mf);

        JMenuBar bar = new JMenuBar();
        bar.setBorder(BorderFactory.createEmptyBorder(4,4,4,4));
        JMenu fileM = bar.add(new JMenu("File"));
        fileM.add(new JMenuItem("New Project..")).addActionListener(l ->  {
            VCmdRunner.runInternal("run", new File(home, "NewProjectDialog.v").getAbsolutePath());
            model.reload();
        });
        ...
        fileM.add(new JMenuItem("Open Terminal")).addActionListener(l -> {
            Term term = new Term(new File("include"));
        ...
        saver.setIcon(new ImageIcon(getImage("icons/save2.png",16,16, 250)));
        ...
        run.setIcon(new ImageIcon(getImage("icons/run.png",16,16, 250)));
        ...
        ImageIcon icon = new ImageIcon(getImage("icons/logo.png", 300,120, 50)); // TODO a
        JLabel label = new JLabel(icon);
        UIManager.put("OptionPane.background", DARK ? Color.DARK_GRAY : Color.WHITE);
        UIManager.put("Panel.background", DARK ? Color.DARK_GRAY : Color.WHITE);
        label.setText("<html>\nVIDE - The IDE for V<br><small>Version: " + VERSION + ", V " + V_FULL + "<br><br>\u00A9 2020-2021 Vide Contibutors.<br>* Not an official product of the makers of V</small></html>");
        label.setHorizontalTextPosition(JLabel.CENTER);
        label.setVerticalTextPosition(JLabel.BOTTOM);
        label.setOpaque(true);
        label.setBackground(DARK ? Color.DARK_GRAY : Color.WHITE);
        label.setFont(label.getFont().deriveFont(14f));

        helpM.add(new JMenuItem("About VIDE")).addActionListener(l -> {
            //JOptionPane.showMessageDialog(null, label, "About VIDE", -1, null);
        ...
        JLabel ap = new JLabel( new ImageIcon(getImage("icons/logo.png", 60,24, 250)) );
        ap.setOpaque(true);
        ...
        Image iconn = getImage("icons/icon.png", 64, 64, 250);
    ...
    public static Image getImage(String s,int a, int b, int i) {
        ...
        try {
            im = ImageIO.read(Vide.class.getClassLoader().getResourceAsStream(s));
        } catch (IOException e){}
    ...
    public static Path exportResource(String res, String outn, File folder) {
        try (InputStream stream = Vide.class.getClassLoader().getResourceAsStream(res)) {
            ...
            Files.copy(stream, p, StandardCopyOption.REPLACE_EXISTING);        System.out.println("EXPORTED!");
            return p;
